...
func (p *Plugin) runWeb(ctx context.Context, webOpts webFlags) error {
	...
	serverOpts := &server.RunOptions{
		...
		APIPrefix:       APIPrefix,
		NoUI:            webOpts.NoUI,
		EnableTokenAuth: webOpts.EnableTokenAuth,
...
func runBackgroundCmd(pidFile string) (int, error) {
	...
	err = createPIDFile(command.Process.Pid, pidFile)
	if err != nil {
...
func createPIDFile(pid int, file string) error {
	err := launchr.EnsurePath(filepath.Dir(file))
	if err != nil {
		return fmt.Errorf("cannot create tmp directory for %q", file)
	}

	err = os.WriteFile(file, []byte(strconv.Itoa(pid)), os.FileMode(0600))
	if err != nil {
