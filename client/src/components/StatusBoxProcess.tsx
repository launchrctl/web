...
const StatusBoxProcess: FC<IStatusBoxProcessProps> = ({ ri, actionId }) => {
  const [streams, setStreams] = useState<components['schemas']['ActionRunStreamData'][]>([])
  const apiUrl = useApiUrl()

  const { refetch: queryRunning } = useCustom<components['schemas']['ActionRunStreamData'][], HttpError>({
