...
func (p *Plugin) DiscoverActions(_ context.Context) ([]*action.Action, error) {
	...
	p.app.Services().Get(&cfg)
	p.app.Services().Get(&k)
	...
	aw.SetRuntime(action.NewFnRuntime(func(ctx context.Context, a *action.Action) error {
		...
		wf := webFlags{
			EnableTokenAuth: input.Opt("enable-token-auth").(bool),
		...
		if err != nil {
			term.Warning().Println("error on getting web.variables %s", err)
		...
		// If 'stop' arg passed, try to interrupt the process and remove a PID file.
		op := input.Arg("op")
		switch op {
		case stopArg:
	...
	at.SetRuntime(action.NewFnRuntime(func(_ context.Context, a *action.Action) error {
		...
		case "list":
			listTokens(tokenStore, a)
			return nil
		case "export":
			return exportTokens(tokenStore, a)
		case "delete":
			return deleteToken(tokenStore, a)
		case "purge":
			purgeTokens(tokenStore, a)
			return nil
		default:
...
func createToken(store *server.TokenStore, a *action.Action) error {
	...
	if info.Expires != nil {
		term.Info().Printfln("Expires: %s", info.Expires.Format("2006-01-02 15:04:05"))
	} else {
...
func listTokens(store *server.TokenStore, a *action.Action) {
	_, term := action.GetLoggerAndTerminal(a)
	...
	for _, token := range tokens {
		term.Info().Printfln("  • %s", token.Name)
		term.Info().Printfln("    Created: %s", token.Created.Format("2006-01-02 15:04:05"))
		if token.Expires != nil {
			term.Info().Printfln("    Expires: %s", token.Expires.Format("2006-01-02 15:04:05"))
		} else {
			term.Info().Printfln("    Expires: Never")
		}
...
func exportTokens(store *server.TokenStore, a *action.Action) error {
	_, term := action.GetLoggerAndTerminal(a)
	...
	yamlData, err := yaml.Marshal(tokens)
	if err != nil {
		return fmt.Errorf("failed to parse tokens: %w", err)
	}

	term.Print(string(yamlData))
	return nil
}
...
func purgeTokens(store *server.TokenStore, a *action.Action) {
	_, term := action.GetLoggerAndTerminal(a)
	exp := store.PurgeInactiveTokens()
	if exp > 0 {
